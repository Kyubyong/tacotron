...
def get_spectrograms(sound_file): 
    ...
    return np.transpose(
        S.astype(np.float32) if hp.log_mel_spectrogram else np.log(S.astype(np.float32) + 1e-10)), np.transpose(
        np.log(magnitude.astype(np.float32) + 1e-10))  # (T, n_mels), (T, 1+n_fft/2)
